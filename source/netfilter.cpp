...
namespace netfilter
{
	...
	bool GetCountryFromAPI(const char* ip, std::string& country) {
		CURL* curl;
		CURLcode res;
		...
		std::string url = "http://localhost:8080/getcountry?ip=" + std::string(ip);
	...
	bool IsFromCISRegion(const char* ip) {
		if (ISPrivateIP(ip)) {
			std::cout << "Private IP: " << ip << std::endl;
			return true;
		}

		std::string country;
		if (GetCountryFromAPI(ip, country)) {
			std::cout << "Country of " << ip << ": " << country << std::endl;

			if (country == "Russia" || country == "Ukraine" || country == "Belarus") {
				return true;
			}
		}
	...
	inline PacketType SendInfoCache(const sockaddr_in &from, uint32_t time)
	{
		...
		if (!IsFromCISRegion(IPToString(from.sin_addr))) {
			printf("%s is not from CIS, aborting request.\n", IPToString(from.sin_addr));
			return PacketTypeInvalid; // ur not from cis? fuck off
		}
